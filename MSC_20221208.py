...
for i in range(int(Total_time/time)):
    ...
    sensor_1 = numda * calculate_mean_value(concentration_1) + (1 - numda) * calculate_Bout(concentration_1) * time
    sensor_2 = numda * calculate_mean_value(concentration_2) + (1 - numda) * calculate_Bout(concentration_2) * time
    sensor_3 = numda * calculate_mean_value(concentration_3) + (1 - numda) * calculate_Bout(concentration_3) * time
    sensor_4 = numda * calculate_mean_value(concentration_4) + (1 - numda) * calculate_Bout(concentration_4) * time
    sensor_5 = numda * calculate_mean_value(concentration_5) + (1 - numda) * calculate_Bout(concentration_5) * time
    sensor_6 = numda * calculate_mean_value(concentration_6) + (1 - numda) * calculate_Bout(concentration_6) * time
...
'''写出插值后的数据'''
np.savetxt('./data/颗粒物-固定传感器采样数据-20221126/MSC_data.txt', raw_data, fmt="%.2f", delimiter=' ')