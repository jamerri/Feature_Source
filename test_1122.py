...
# 生成随机浓度数 0-1
for i in range(112):
    c.append('%.2f' % random.random())

# print(c)

# 读取数据
book = load_workbook(filename=r"pm_robot.xlsx")  # 文件路径
sheet = book["Sheet1"]  # 读取名字为Sheet1的表

row_num = 2
while row_num <= 113:
    '''将表中第 i 列的2-113行数据写入数组中'''
    x_origin.append('%.2f' % (4.97 - sheet.cell(row=row_num, column=2).value))
    row_num = row_num + 1

row_num = 2
while row_num <= 113:
    '''将表中第 i 列的2-113行数据写入数组中'''
    y_origin.append('%.2f' % (2.38 + sheet.cell(row=row_num, column=1).value))
    row_num = row_num + 1
...
'''写出插值后的数据'''
np.savetxt('interpolation_data.txt', interpolation_data, fmt="%.2f", delimiter=' ')
...
# 保存图片
plt.savefig('test_1122.tif', bbox_inches='tight', dpi=600)
plt.show()
